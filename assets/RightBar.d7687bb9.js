import {
  p as s,
  f as a,
  C as l,
  o as e,
  c,
  k as n,
  m as i,
  h as A,
  l as g,
  B as d,
  s as t,
  D as o,
  F as v,
} from "./vendor.ba249473.js";
var u = "./assets/logo.d7845fb0.png";
s("data-v-18376f08");
const r = { class: "menu-panel" },
  k = { class: "menu-header" },
  p = ["onClick"],
  C = [
    n(
      "img",
      { src: "./assets/logo.d7845fb0.png", alt: "", draggable: "false" },
      null,
      -1,
    ),
  ],
  f = n("span", null, "目 录", -1),
  E = { class: "grid" },
  b = n(
    "div",
    { class: "cell" },
    [
      n("div", { class: "cell-inner" }, [
        n("div", { class: "icon" }, [
          n("img", {
            src: "./assets/a6.097be1d1.png",
            alt: "",
            draggable: "false",
          }),
        ]),
        n("div", { class: "texts" }, [
          n("div", { class: "title" }, "生态"),
          n("div", { class: "subs" }, [
            n("a", null, "创作者生态"),
            n("i", { class: "sep" }),
            n("a", null, "物件生态"),
            n("i", { class: "sep" }),
            n("a", null, "经济生态"),
          ]),
        ]),
      ]),
    ],
    -1,
  ),
  x = n(
    "div",
    { class: "cell" },
    [
      n("div", { class: "cell-inner" }, [
        n("div", { class: "icon" }, [
          n("img", {
            src: "./assets/a7.55faabb4.png",
            alt: "",
            draggable: "false",
          }),
        ]),
        n("div", { class: "texts" }, [
          n("div", { class: "title" }, "财务"),
          n("div", { class: "subs" }, [
            n("a", null, "质押"),
            n("i", { class: "sep" }),
            n("a", null, "NFT商城"),
          ]),
        ]),
      ]),
    ],
    -1,
  ),
  S = { class: "cell" },
  R = { class: "cell-inner" },
  U = n(
    "div",
    { class: "icon" },
    [
      n("img", {
        src: "./assets/a8.1d9f5f76.png",
        alt: "",
        draggable: "false",
      }),
    ],
    -1,
  ),
  m = { class: "texts" },
  w = n("div", { class: "title" }, "关于我们", -1),
  B = { class: "subs" },
  G = ["onClick"],
  I = n("i", { class: "sep" }, null, -1),
  O = n("a", null, "项目介绍", -1),
  T = n("i", { class: "sep" }, null, -1),
  W = n("a", null, "合作伙伴", -1),
  J = n("i", { class: "sep" }, null, -1),
  P = n("a", null, "团队介绍", -1),
  y = n(
    "div",
    { class: "cell" },
    [
      n("div", { class: "cell-inner" }, [
        n("div", { class: "icon" }, [
          n("img", {
            src: "./assets/a9.a071301b.png",
            alt: "",
            draggable: "false",
          }),
        ]),
        n("div", { class: "texts" }, [n("div", { class: "title" }, "市场")]),
      ]),
    ],
    -1,
  ),
  h = n(
    "div",
    { class: "cell" },
    [
      n("div", { class: "cell-inner" }, [
        n("div", { class: "icon" }, [
          n("img", {
            src: "./assets/a10.a9ec1a1f.png",
            alt: "",
            draggable: "false",
          }),
        ]),
        n("div", { class: "texts" }, [
          n("div", { class: "title" }, "常见问题"),
        ]),
      ]),
    ],
    -1,
  ),
  N = n(
    "div",
    { class: "cell" },
    [
      n("div", { class: "cell-inner" }, [
        n("div", { class: "icon" }, [
          n("img", {
            src: "./assets/a11.37aed859.png",
            alt: "",
            draggable: "false",
          }),
        ]),
        n("div", { class: "texts" }, [
          n("div", { class: "title" }, "合规信息"),
        ]),
      ]),
    ],
    -1,
  ),
  Q = n(
    "footer",
    { class: "menu-footer" },
    [
      n("div", { class: "links" }, [
        n("a", null, "使用条款"),
        n("a", null, "隐私政策"),
        n("a", null, "联系方式"),
      ]),
    ],
    -1,
  );
a();
const z = {
  emits: ["close"],
  setup(s, { emit: a }) {
    const A = l();
    function g(s) {
      s.target === s.currentTarget && a("close");
    }
    function d() {
      (A.push("/about"), a("close"));
    }
    function t() {
      a("close");
    }
    return (s, a) => (
      e(),
      c("div", { class: "menu-mask", onClick: g }, [
        n("div", r, [
          n("header", k, [
            n("div", { class: "logoh", onClick: i(t, ["stop"]) }, C, 8, p),
            f,
          ]),
          n("div", E, [
            b,
            x,
            n("div", S, [
              n("div", R, [
                U,
                n("div", m, [
                  w,
                  n("div", B, [
                    n("a", { onClick: i(d, ["stop"]) }, "公司介绍", 8, G),
                    I,
                    O,
                    T,
                    W,
                    J,
                    P,
                  ]),
                ]),
              ]),
            ]),
            y,
            h,
            N,
          ]),
          Q,
        ]),
      ])
    );
  },
  __scopeId: "data-v-18376f08",
};
var V =
    "data:image/png;base64,",
  j =
    "data:image/png;base64,",
  D =
    "data:image/png;base64,";
s("data-v-f115fe6a");
const L = { class: "right-bar" },
  H = n(
    "div",
    { class: "notice" },
    [
      n("img", {
        src: "data:image/png;base64,",
        alt: "",
        draggable: "false",
      }),
    ],
    -1,
  ),
  F = n("div", { class: "divider" }, null, -1),
  Z = { class: "slogan" },
  X = { class: "langs" },
  Y = { key: 0, class: "dot ja" },
  q = [n("img", { src: V, alt: "", draggable: "false" }, null, -1)],
  M = { key: 1, class: "dot en" },
  K = [n("img", { src: j, alt: "", draggable: "false" }, null, -1)],
  _ = { key: 2, class: "dot zh" },
  $ = [n("img", { src: D, alt: "", draggable: "false" }, null, -1)],
  ss = [
    n(
      "span",
      { class: "dot ja" },
      [n("img", { src: V, alt: "", draggable: "false" })],
      -1,
    ),
    n("span", null, "日本語", -1),
  ],
  as = [
    n(
      "span",
      { class: "dot en" },
      [n("img", { src: j, alt: "", draggable: "false" })],
      -1,
    ),
    n("span", null, "EN", -1),
  ],
  ls = [
    n(
      "span",
      { class: "dot zh" },
      [n("img", { src: D, alt: "", draggable: "false" })],
      -1,
    ),
    n("span", null, "中文", -1),
  ];
a();
const es = {
  props: {
    slogan: { type: String, default: "开启您的未来之旅" },
    fabOpen: { type: Boolean, default: !1 },
    showLangs: { type: Boolean, default: !1 },
  },
  emits: ["fab-click"],
  setup(s, { emit: a }) {
    const l = s,
      i = A(!1),
      u = A("zh");
    function r() {
      i.value = !i.value;
    }
    function k(s) {
      ((u.value = s), (i.value = !1));
    }
    return (s, A) => (
      e(),
      c("aside", L, [
        H,
        F,
        g(s.$slots, "extra", {}, void 0, !0),
        n("div", Z, d(l.slogan), 1),
        n(
          "button",
          {
            class: t(["fab", { open: l.fabOpen }]),
            onClick: A[0] || (A[0] = (s) => a("fab-click")),
          },
          null,
          2,
        ),
        n("div", X, [
          i.value
            ? (e(),
              c(
                v,
                { key: 1 },
                [
                  n(
                    "div",
                    { class: "lang", onClick: A[1] || (A[1] = (s) => k("ja")) },
                    ss,
                  ),
                  n(
                    "div",
                    { class: "lang", onClick: A[2] || (A[2] = (s) => k("en")) },
                    as,
                  ),
                  n(
                    "div",
                    { class: "lang", onClick: A[3] || (A[3] = (s) => k("zh")) },
                    ls,
                  ),
                ],
                64,
              ))
            : (e(),
              c("div", { key: 0, class: "lang", onClick: r }, [
                "ja" === u.value ? (e(), c("span", Y, q)) : o("", !0),
                "en" === u.value ? (e(), c("span", M, K)) : o("", !0),
                "zh" === u.value ? (e(), c("span", _, $)) : o("", !0),
                n(
                  "span",
                  null,
                  d(
                    "ja" === u.value
                      ? "日本語"
                      : "en" === u.value
                        ? "EN"
                        : "中文",
                  ),
                  1,
                ),
              ])),
        ]),
      ])
    );
  },
  __scopeId: "data-v-f115fe6a",
};
export { u as _, es as a, z as b };
